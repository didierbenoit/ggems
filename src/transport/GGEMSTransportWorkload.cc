...
#include "GGEMS/observer/GGEMSObserverCounterArithmetic.hh"
...
ComputeObserverSafeLaunchPrimaryCount(std::uint32_t transport_limit,
  ...
  std::uint32_t const observer_limit =
    std::numeric_limits<std::uint32_t>::max() / k_observer_records_per_primary;
  ...
  for (std::size_t source_index = 0U;
    ...
    for (std::uint64_t emission_index = next_emission_index;
      ...
      SourceEmissionRange const &emission_range =
        config.source_emission_ranges[static_cast<std::size_t>(emission_index)];
...
GGEMSTransportWorkload::GGEMSTransportWorkload(
  ...
  std::uint32_t launch_primary_count_limit)
    : context_{&context}, kernel_root_{std::move(kernel_root)},
      ...
      source_emission_records_buffer_{context.CreateSVMBuffer(
        ComputeArrayBufferSize(emission_count_, sizeof(SourceEmissionRecord)))},
      source_emission_ranges_buffer_{context.CreateSVMBuffer(
      ...
      observer_counters_buffer_{
        context.CreateSVMBuffer(ggems::units::Bytes{sizeof(ObserverCounters)})},
...
auto GGEMSTransportWorkload::Run(GGEMSTransportRunConfig const &config)
  ...
  ggems::ocl::WriteSVMFromHost(
    source_population_records_buffer_,
    std::span<SourcePopulationRecord const>{config.source_population_records});
