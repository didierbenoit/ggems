...
namespace ggems::core {
  static std::string FormatTimestamp(std::chrono::system_clock::time_point const& tp) {
  ...
  std::string LogFormatter::Format(LogRecord const& rec, LogColorTheme const& theme, bool use_colour) const {
    char const* level_str = "INFO";
    char const* col = "";
    char const* reset = "";

    switch (rec.level_) {
      case LogLevel::Debug: level_str = "DEBUG"; col = theme.debug_.c_str(); break;
      case LogLevel::Info:  level_str = "INFO";  col = theme.info_.c_str();  break;
      case LogLevel::Warn:  level_str = "WARN";  col = theme.warn_.c_str();  break;
      case LogLevel::Error: level_str = "ERROR"; col = theme.error_.c_str(); break;
    }
    ...
    auto const tid = std::hash<std::thread::id>{}(rec.thread_id_);
    ...
    return std::vformat(
      ...
      std::make_format_args(
        col, ts, level_str, tid, reset, module_part,
        rec.function_, rec.message_)
    );
  }
  ...
  bool GGEMSLogger::UseColour() const noexcept {
    if (force_colour_.has_value()) return *force_colour_;
  ...
  void GGEMSLogger::Dispatch(LogRecord const& rec) {
    ...
    std::lock_guard<std::mutex> lock(mtx_);
    for (auto& s : sinks_) s->Write(rec, line);
  }
} // namespace ggems::core
