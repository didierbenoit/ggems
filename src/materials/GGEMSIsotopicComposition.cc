...
constexpr auto k_reference_isotopes = std::array{
  ...
  MakeReferenceIsotope(86U, 222U, 0U), MakeReferenceIsotope(87U, 223U, 0U),
  MakeReferenceIsotope(88U, 226U, 0U), MakeReferenceIsotope(89U, 227U, 0U),
  MakeReferenceIsotope(93U, 237U, 0U), MakeReferenceIsotope(94U, 244U, 0U),
  MakeReferenceIsotope(95U, 243U, 0U), MakeReferenceIsotope(96U, 247U, 0U),
  MakeReferenceIsotope(97U, 247U, 0U), MakeReferenceIsotope(98U, 251U, 0U),
  MakeReferenceIsotope(99U, 252U, 0U),
};
...
[[nodiscard]] auto FindNaturalIsotopes(std::uint32_t atomic_number) noexcept
  ...
  auto const rows = std::ranges::equal_range(
    k_nist41_natural_isotopes, atomic_number, {},
    [](NaturalIsotopeRow const &row) noexcept -> std::uint32_t {
...
[[nodiscard]] auto FindReferenceIsotope(std::uint32_t atomic_number) noexcept
  ...
  return found == k_reference_isotopes.end() ? nullptr : &*found;
}
...
[[nodiscard]] auto BuildDefaultIsotopicComposition(std::uint32_t atomic_number)
  ...
  for (auto const &row : FindNaturalIsotopes(atomic_number)) {
    fractions.push_back({
      .isotope = row.isotope,
  ...
  if (auto const *isotope = FindReferenceIsotope(atomic_number);
      isotope != nullptr) {
    return GGEMSIsotopicComposition{GGEMSFractionBasis::AtomFraction,
                                    {{.isotope = *isotope, .fraction = 1.0L}}};
  }

  throw GGEMSRecoverable{
    std::format("No default isotopic composition for Z={}.", atomic_number)};
}
