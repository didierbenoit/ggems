...
constexpr std::array<ExpectedBetaBranch, 6U> k_expected_beta_branches{{
    {.yield_per_decay = 0.02130L,
     ...
     .endpoint_milli_eV = 247'900'000ULL,
     .represented_mean_energy_keV = 69.36982},
    {.yield_per_decay = 0.00643L,
     .table_count = 608U,
     .bin_width_milli_eV = 499'834ULL,
     .lower_edge_milli_eV = 928ULL,
     .endpoint_milli_eV = 303'900'000ULL,
     .represented_mean_energy_keV = 86.97934},
    {.yield_per_decay = 0.0720L,
     .table_count = 668U,
     .bin_width_milli_eV = 499'700ULL,
     .lower_edge_milli_eV = 400ULL,
     .endpoint_milli_eV = 333'800'000ULL,
     .represented_mean_energy_keV = 96.63034},
    {.yield_per_decay = 0.894L,
     .table_count = 1213U,
     .bin_width_milli_eV = 499'834ULL,
     .lower_edge_milli_eV = 1'358ULL,
     .endpoint_milli_eV = 606'300'000ULL,
     .represented_mean_energy_keV = 191.90012},
    {.yield_per_decay = 0.0006L,
     .table_count = 1260U,
     .bin_width_milli_eV = 499'760ULL,
     .lower_edge_milli_eV = 2'400ULL,
     .endpoint_milli_eV = 629'700'000ULL,
     .represented_mean_energy_keV = 200.26196},
    {.yield_per_decay = 0.00386L,
     .table_count = 1614U,
     .bin_width_milli_eV = 499'938ULL,
     .lower_edge_milli_eV = 68ULL,
     .endpoint_milli_eV = 806'900'000ULL,
...
TEST(GGEMSI131Test, PreservesLaraAndMirdAtomicRadiations) {
  ...
  EXPECT_EQ(auger_energies.front(), 26'150ULL);
  EXPECT_EQ(auger_energies[1U], 36'237ULL);
  EXPECT_EQ(auger_energies[7U], 3'324'020ULL);
  EXPECT_EQ(auger_energies.back(), 32'903'200ULL);
  ...
  EXPECT_EQ(conversion_energies.front(), 45'621'000ULL);
  EXPECT_EQ(conversion_energies[21U], 249'741'000ULL);
  EXPECT_EQ(conversion_energies[63U], 329'925'600ULL);
  EXPECT_EQ(conversion_energies[86U], 602'425'600ULL);
  EXPECT_EQ(conversion_energies.back(), 722'780'700ULL);
...
TEST(GGEMSI131Test, ExcludesDelayedXe131mDeexcitation) {
  ...
  auto const gamma_energies =
      emissions[6U].GetEnergyDistribution().GetEnergyValuesMilliElectronVolt();
  for (std::uint64_t const energy : gamma_energies) {
    EXPECT_NE(energy, 163'930'000ULL);
  }

  auto const conversion_energies =
      emissions[9U].GetEnergyDistribution().GetEnergyValuesMilliElectronVolt();
  for (std::uint64_t const energy : conversion_energies) {
    EXPECT_NE(energy, 129'366'000ULL);
    EXPECT_NE(energy, 158'477'000ULL);
    EXPECT_NE(energy, 158'826'000ULL);
    EXPECT_NE(energy, 159'148'000ULL);
    EXPECT_NE(energy, 163'038'000ULL);
    EXPECT_NE(energy, 163'802'000ULL);
  }
}
