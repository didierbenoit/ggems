...
constexpr std::array<ExpectedBetaBranch, 3U> k_expected_beta_branches{{
    {.yield_per_decay = 0.9988L,
     .table_count = 635U,
     .bin_width_milli_eV = 499'716ULL,
     .lower_edge_milli_eV = 340ULL,
     .endpoint_milli_eV = 317'320'000ULL,
     .represented_mean_energy_keV = 95.52460},
    {.yield_per_decay = 0.00002L,
     .table_count = 1329U,
     .bin_width_milli_eV = 499'968ULL,
     .lower_edge_milli_eV = 2'528ULL,
     .endpoint_milli_eV = 664'460'000ULL,
     .represented_mean_energy_keV = 274.31370},
    {.yield_per_decay = 0.0012L,
     .table_count = 2982U,
     .bin_width_milli_eV = 499'852ULL,
     .lower_edge_milli_eV = 1'336ULL,
     .endpoint_milli_eV = 1'490'560'000ULL,
...
TEST(GGEMSCo60Test, PreservesLaraAndMirdAtomicRadiations) {
  ...
  EXPECT_EQ(x_ray_energies[0U], 840'000ULL);
  EXPECT_EQ(x_ray_energies[1U], 7'460'970ULL);
  EXPECT_EQ(x_ray_energies[2U], 7'478'240ULL);
  EXPECT_EQ(x_ray_energies[3U], 8'296'700ULL);
  ...
  EXPECT_EQ(auger_energies.front(), 47'980ULL);
  EXPECT_EQ(auger_energies[2U], 772'593ULL);
  EXPECT_EQ(auger_energies[4U], 6'497'880ULL);
  EXPECT_EQ(auger_energies.back(), 8'133'490ULL);
  ...
  EXPECT_EQ(conversion_energies.front(), 338'810'000ULL);
  EXPECT_EQ(conversion_energies[4U], 1'164'907'200ULL);
  EXPECT_EQ(conversion_energies[6U], 1'324'175'200ULL);
  EXPECT_EQ(conversion_energies[8U], 2'150'277'000ULL);
  EXPECT_EQ(conversion_energies.back(), 2'504'836'000ULL);
